...
PostProcessing {
	{ Name PostMain; NameOfFormulation FrmV;
		Quantity {
			...
			{ Name EexSqu; Value {Integral {Type Global; // E^2 exact integral
				[ coef* SquNorm[Eex[]] ];
				Integration I2; Jacobian J1; In VolVacInt;}}
			}
...
PostOperation {
	{ Name PostMain; NameOfPostProcessing PostMain; 
		Format Table;
		Operation {
			Print[{prob, quarters, bound, s, ImposeV}, Format "Prob=%g, Quarters=%g, Bound=%g, s=%g, ImposeV=%g:", File > "output.txt"]; 

			Print[ L2error, OnGlobal, StoreInVariable $L2error ];
			Print[ EexSqu, OnGlobal, StoreInVariable $EexSqu ];
			Print[ {Sqrt[$L2error/$EexSqu]}, Format 
