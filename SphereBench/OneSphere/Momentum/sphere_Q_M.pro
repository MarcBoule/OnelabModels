...
// TODO: improve efficiency, in order to have only one electrostatics resolution (currently solves FrmV0 first for floating potential of sphere, then solve it again using this potential as weak dirichlet so that Lagrange mutlipler can be used for charge density)
// Note: if no momentum, then remove 2nd pass with lag mult and use We3 = 0.5*Q*V in place of We2, but for Lc2, we really need charge density via lambda since A not constant
...
FunctionSpace {
	{ Name HgradV0; Type Form0; // electric scalar potential
		BasisFunction {
...
Formulation {
	{ Name FrmV0; Type FemEquation;
		Quantity {
			{ Name v; Type Local; NameOfSpace HgradV0; }
			{ Name U; Type Global; NameOfSpace HgradV0[GlobalVs]; }
			{ Name Q; Type Global; NameOfSpace HgradV0[GlobalQs]; }
		}
		Equation {
	...
	{ Name FrmV; Type FemEquation;
		Quantity {
			...
			{ Name v0; Type Local; NameOfSpace HgradV0; }
...
Resolution {
	{ Name ResMain;
		System {
			{ Name SV0; NameOfFormulation FrmV0; }
		...
		Operation {
			Generate[SV0]; Solve[SV0]; SaveSolution[SV0];
...
PostProcessing {
	...
	{ Append; Name PostMain;
		Quantity {
			...
			// [ coef*Cross[ r[], {lam} * {a} ] ]; // needs Coulomb gauging when {lam} (i.e. sigma_f) not uniform; assumes sigma=sigma_free
			...
			{ Name Lc2; Value {Integral {Type Global;
				[ coef*Cross[ r[], {lam} * {a} ] ]; // needs Coulomb gauging when {lam} (i.e. sigma_f) not uniform; assumes sigma=sigma_free; Lagrange mult or mixed formulation not needed since vecA happens to be parallel to surface, so we can use vecA directly in surface integral
				Integration I1; Jacobian J1; In SurSphere;}}
			}
...
PostOperation {
	{ Name PostMain; NameOfPostProcessing PostMain; 
		...
		Operation {
			...
			Print[ We3, OnRegion SurSphere, StoreInVariable $We3 ];
			Print[ {$We3, We[], ($We3-We[])/We[]*10^6}, Format 
			" We3 = %.8g [J] (analyt %.8g, %.3g ppm)", File > "output.txt" ];
