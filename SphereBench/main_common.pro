...
// sphere:
rs = 1.0*cm;     // radius
xs = 0*cm * (quarters < 2); // x position of center
ys = 0*cm * (quarters < 4); // y position of center
zs = -2*cm;      // z position of center

// cylinder:
rc = 1.0*cm;     // radius
hc = 4*cm;       // height
xc = 0*cm * (quarters < 2); // x position of center
yc = 0*cm * (quarters < 4); // y position of center
