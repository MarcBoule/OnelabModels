...
quarters = 4;    // 1 = quarter-of-domain, 2 = half, 4 = full
...
// sphere:
rs = 1.0*cm;     // radius
xs = 1*cm * (quarters > 1); // x position of center
ys = 1*cm * (quarters > 2); // y position of center
zs = -2*cm;      // z position of center

// cylinder:
rc = 1.0*cm;     // radius
hc = 4*cm;       // height
xc = 1*cm * (quarters > 1); // x position of center
yc = 1*cm * (quarters > 2); // y position of center
